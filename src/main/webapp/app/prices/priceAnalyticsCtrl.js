    ...
    function PriceAnalyticsCtrl($scope,$filter, products, productService){
        ...
        for (var i = 0; i < products.length; i++) {
            products[i].marginPercent =
                productService.calculateMarginPercent(products[i].current,
                                                        products[i].previous);
            products[i].marginAmount =
                productService.calculateMarginAmount(products[i].current,
                                                        products[i].previous);
        }
        ...
        for (var i = 0; i < filteredProductsAmount.length; i++) {
            chartDataAmount.push({
                ...
                y: [filteredProductsAmount[i].previous,
                    filteredProductsAmount[i].current,
                    filteredProductsAmount[i].marginAmount]
            });
        }

        $scope.dataAmount = {
            series: ["previous", "current", "Margin Amount"],
            data: chartDataAmount
        };

        $scope.configAmount = {
            title: "Top  Margin Item",
            tooltips: true,
            labels: false,
            mouseover: function () { },
            mouseout: function () { },
